...
const useChatStore = create<ChatStore>((set, get) => ({
    ...
    initSocketListeners: () => {
        ...
        conversations?.forEach((conversation) => {
            ...
            socket.emit("get_user_status", otherUser._id, (status:UserStatus) => {
                set((state) => {
                    const newOnlineUsers = new Map(state.onlineUsers);
    ...
    fetchConversations: async () => {
        ...
        try {
            const { data } = await axiosInstance.get<Conversation[]>("/chats/conversations");
